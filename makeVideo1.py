...
class dictGraph1(Scene):
    def construct(self):
        ...
        paragraph2 = Paragraph("If two words differ by one", "character, connect them.",
                               font_size=22, line_spacing=0.5)
        paragraph3 = Paragraph("We will end up with a graph.", font_size=22)
        paragraph4 = Paragraph("Now do this for the 20,000 most",
                               "common words in English and Russian,",
                               font_size=22, line_spacing=0.5)
        paragraph5 = Paragraph("because why not.", font_size=22)
        VGroup(paragraph1, paragraph2, paragraph3, paragraph4, paragraph5).arrange(DOWN, buff=0.1, aligned_edge=LEFT).move_to(RIGHT*4)
